...
int main(){
	...
	for(int channels = 0 ; channels < 3 ; channels++){
		...
		for(int i = 0 ; i<3 ; i++){
			Mat res = RollingGuidanceFilter::filter(img,R[i],25.5,4);
			imwrite("./imgs_b/roll" + "_ch_" + to_string(channel) + to_string(i) + ".jpg" , res);
			Mat enis = (img - res)*5 + res;
			imwrite("./imgs_b/enh" + "_ch_" + to_string(channel) +to_string(i) + ".jpg" , enis);

			Mat resc = RollingGuidanceFilter::filter(color,R[i],25.5,4);
			imwrite("./imgs_b/roll_color" + "_ch_" + to_string(channel) +to_string(i) + ".jpg" , resc);
			Mat enisc = (color - resc)*5 + 1*resc;
			imwrite("./imgs_b/enh_color" + "_ch_" + to_string(channel) +to_string(i) + ".jpg" , enisc);
			...
			imwrite("./imgs_b/laplace"+"_ch_" + to_string(channel) +to_string(i+1) +".jpg",res);
			float* r8 = createHist(res);
			...
			for(int j = 0; j<16 ; j++){
				...
				imwrite("./imgs_b/fi_"+"_ch_" + to_string(channel) +to_string(i)+"_"+to_string(j)+".jpg",fi[j]);
				GaussianBlur( fi[j], fi[j],Size(0,0), R[i]);
			...
			imwrite("./imgs_b/m_"+"_ch_" + to_string(channel) +to_string(i)+".jpg",M[i]);
		...
		imwrite("./imgs_b/output"  + "_ch_" + to_string(channel) +".jpg" , output); // M map
		Mat I_basic = RollingGuidanceFilter::filter(img,1,25.5,1);
		imwrite("./imgs_b/I_basic" + "_ch_" + to_string(channel) +".jpg" , I_basic);
		int **IoM_arr;
		...
		imwrite("./imgs_b/IoM" + "_ch_" + to_string(channel) + ".jpg" , IoM_);
		printf("check point 5");
		Mat I_det = I_basic - IoM_;
		imwrite("./imgs_b/I_det" +"_ch_" + to_string(channel) + ".jpg" , I_det);
